...
public class AuthFilter implements Filter{
	...
	private String api_scopes = "com.webage.data.apis";
	private String auth_scopes = "com.webage.auth.apis";
	...
	throws IOException, ServletException {
		...
		if (uri.startsWith("/token")) {
			...
			System.out.println("Check JWT Token from header [" + authheader + "]");
			if (authheader != null && authheader.length() > 7
				&& authheader.startsWith("Bearer")) {
				String jwt_token = authheader.substring(7, authheader.length());
				if (jwtUtil.verifyToken(jwt_token)) {
					String request_scopes = jwtUtil.getScopes(jwt_token);
					if (request_scopes.contains(api_scopes) ||request_scopes.contains(auth_scopes) ) {
						// continue on to api
						chain.doFilter(request, response);
						return;
					}
